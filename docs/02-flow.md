...
**本项目没有"中央状态机校验函数"。** 拦截分散在四层，从强到弱：

| # | 防线 | 拦什么 | 在哪一层 |
|---|---|---|---|
| 1 | **WHERE 守卫**：每条转移 SQL 都带状态前置条件 | 时序错误 + 并发竞态（唯一真正有效的一道） | SQL |
| 2 | **`RowsAffected() != 1` 判定** | 让"没转成"变成可观测结果，而不是静默失败 | Go 代码 |
| 3 | **`CHECK (status IN (...))`** | 状态值拼写错误（库里永不出现第六个值） | 表结构 |
| 4 | **状态机测试**（W14 的 4 个测试） | 回归：改动之后非法转换仍然被拒 | 测试 |

#### 防线 1 是主力：守卫写在 WHERE 里

状态流转**没有一个通用函数**，每个业务动作有自己专用的 SQL，且都带状态前置条件：

| 动作 | SQL 的 WHERE 条件 | 天然排除了什么 |
|---|---|---|
| 抢占 | `status IN ('pending','retrying')` | `delivered` / `failed` / 正被占用的 |
| 投递结算 | `status = 'processing'` | 已被回收或已被结算的 |
| 租约回收 | `status = 'processing' AND locked_until < now` | 正在投递的 |
| 手动重试 | `status = 'failed'` | `delivered`（正是业务规则 B11 要拦的） |

> **"`delivered` 的事件不可能被改回 `pending`"，不是因为有段 `if` 判断，而是因为代码里根本不存在一条 WHERE 能匹配到 `delivered` 的、指向 `pending` 的语句。**

两个顺带拿到的好处（都是 WHERE 守卫的副产品）：

- **互斥性天然成立**：手动重试要求 `failed`、租约回收要求 `processing`，一行不可能同时满足，所以"用户点重试"和"后台回收"不会打架。
- **幂等性天然成立**：同一动作执行两次，第二次守卫匹配不上 → 影响 0 行 → 无副作用。重复投递一次不会把状态改坏。

#### 防线 2：SQLite 改 0 行时不会报错

`UPDATE` 没匹配到任何行时，SQLite **不报错**，它返回"成功，影响 0 行"。不检查就会变成**"状态没变，但程序以为变了"**——这是最危险的一类 bug，因为日志干干净净。

```go
res, _ := tx.Exec(claimSQL, ...)
n, _ := res.RowsAffected()
if n != 1 {
    // 抢占场景：被别人抢先了 → 正常，不是错误，回去捞下一条（记 debug 级日志）
    // 手动重试场景：这条事件不是 failed（可能已 delivered）→ 必须给用户回 409
}
```

**拦截的具体动作就发生在这一句 `if`。** 注意两个场景的处理方式完全不同：一个是正常的并发竞争，一个是要告知用户的边界拒绝。

#### 防线 3 拦的是另一类错

`CHECK` 约束拦不住"合法值之间的非法跳转"（比如把 `delivered` 直接 `UPDATE` 成 `pending`），它拦的是**代码里拼错的状态字符串**（写成 `"succeed"`、`"deliverd"`）。两类错误要分清：

- **CHECK 拦**：值不在枚举里 → 语法错误
- **WHERE 守卫拦**：值合法但当前状态不允许 → 时序错误

#### 为什么不做 `canTransition(from, to) bool`

看着更像"状态机"，但在 V0.1 是纯负担，三个理由：

1. **它拦不住并发**：校验通过之后、`UPDATE` 执行之前，另一个 worker 可能已经把状态改了——典型的 check-then-act 竞态（TOCTOU）。**校验和修改必须是同一条原子语句**，也就是防线 1 那套。
2. **它制造双真相**：转移规则同时写在 Go 的 map 和 SQL 的 WHERE 里，改了一处忘了另一处，比不写更糟。
3. **它只是给开发者的便利**，不是安全防线。

转移表以**文档和测试数据**的形式存在（就是 3.2 那张表），不放在运行时的写路径上。

#### 一条评审红线

代码评审时只问一句：**所有修改 `events.status` 的语句，是否都带了状态前置条件？** 没有例外，带了才过。W14 写测试前先做一次全量扫描，把这批 SQL 和它们的前置条件列成一张表逐一核对。
